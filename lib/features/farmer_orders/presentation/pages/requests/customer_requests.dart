...
import 'package:customer_app/features/farmer_home/presentation/pages/home/shipments.dart';
  ...
  void _updateStatus(String orderId, String newStatus) async {
    try {
      ...
      if (newStatus == 'confirmed') {
        updates['shippingStatus'] = 'Preparing';
      }
      ...
      await FirebaseFirestore.instance.collection('activities').add({
        ...
        'description':
            'Farmer ${user?.fullName} $newStatus a customer request.',
        'type': 'order_update',
      ...
      if (newStatus == 'confirmed') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text("Order confirmed! Added to Shipments."),
              ...
              action: SnackBarAction(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => ShipmentsPage(uid: user?.uid),
                    ),
                  );
                },
              ),
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance
                  ...
                  .where(
                    'status',
                    isEqualTo: _selectedTab == 0 ? 'pending' : 'delivered',
                  )
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final cropName = items.isNotEmpty
                        ? (items[0]['name'] ?? 'Crop')
                        : 'Crop';
                    final quantity = items.isNotEmpty
                        ? "${items[0]['quantity']} ${items[0]['unit'] ?? 'kg'}"
                        : "1 kg";
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: const Color.fromRGBO(156, 229, 101, 1),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(10),
                      ),
                    ),
