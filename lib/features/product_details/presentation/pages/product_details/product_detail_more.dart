...
class BuildTitleSection extends StatefulWidget {
  ...
  final String userId;
  const BuildTitleSection({super.key, required this.product, required this.userId});
...
class _BuildTitleSectionState extends State<BuildTitleSection> {
  ...
  Future<void> _toggleFav() async {
    ...
    if (_isFav) {
      await ref.set({'productId': widget.product.id, 'addedAt': FieldValue.serverTimestamp()});
    } else {
      await ref.delete();
    }
  }
        ...
        Padding(
          ...
          child: Column(
            ...
            children: [
                  ...
                  GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          width: 1.0,
                          color: _isFav ? Colors.red.shade200 : const Color.fromRGBO(0, 0, 0, 0.2),
                        ),
                      ),
                      child: Center(
                        child: Icon(
              ...
              FeatureCard(Icons.eco, '100% Organic Certified', 'No synthetic pesticides used'),
              const SizedBox(height: 10),
              FeatureCard(Icons.local_shipping, 'Farm to Door Delivery', 'Fresh from the farm daily'),
              const SizedBox(height: 20),
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              child: const Icon(Icons.person, color: Color(0xff0F5700), size: 28),
            ),
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Sold by', style: TextStyle(color: Colors.grey, fontSize: 11)),
                  Text(
                    product.farmerName.isNotEmpty ? product.farmerName : 'Unknown Farmer',
              ...
              child: Container(
                ...
                child: const Row(
                  children: [
                    ...
                    SizedBox(width: 6),
                    Text('Message', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
            ),
...
class FarmerProfilePage extends StatelessWidget {
  ...
  const FarmerProfilePage({super.key, required this.farmerId, required this.farmerName});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        stream: FirebaseFirestore.instance.collection('users').doc(farmerId).snapshots(),
        builder: (context, snapshot) {
          final data = snapshot.data?.data() as Map<String, dynamic>?;
          final phone = data?['phone'] ?? data?['mobile'] ?? '0';
          final place = data?['place'] ?? data?['extraData']?['place'] ?? 'Kerala, India';
                    ...
                    children: [
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            const Expanded(
                              child: Text('Farmer Profile', textAlign: TextAlign.center,
                      ...
                      CircleAvatar(
                        ...
                        child: profileImg.isEmpty
                            ? Text(farmerName.isNotEmpty ? farmerName[0].toUpperCase() : 'F',
                                style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Color(0xff0F5700)))
                            : null,
                      ),
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 10),
                      _infoCard(Icons.location_on, 'Location', place.toString()),
                      const SizedBox(height: 10),
                      _infoCard(Icons.badge_outlined, 'Farmer ID', farmerId.isNotEmpty ? farmerId : '0'),
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () {
                            Navigator.push(context, MaterialPageRoute(
                              builder: (context) => ChatPage(farmerId: farmerId, farmerName: farmerName),
                            ));
                          },
                          icon: const Icon(Icons.chat_bubble_outline, color: Colors.white),
                          label: const Text('Send Message', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            Text(subtitle, style: const TextStyle(fontWeight: FontWeight.w400, fontSize: 14)),
          ],
        ),
      ],
    ),
